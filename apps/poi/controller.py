...
class POIController:
    def __init__(self, db_client: AsyncIOMotorClient, db_name: str):
        ...
        self.collection = self.db.poi_persons
        self.watchlist_collection = self.db.groups  # Reference to watchlist collection
    ...
    async def create_poi(self, poi_data: POICreate, created_by_user_id: str) -> POIResponse:
        """Create a new POI"""
        try:
            # Check if watchlist exists if provided
            if poi_data.tagged_watchlist_id:
                watchlist = await self.watchlist_collection.find_one({
                    "_id": ObjectId(poi_data.tagged_watchlist_id),
                    "organization_id": poi_data.organization_id,
                    "is_active": True
                })
                if not watchlist:
                    raise HTTPException(
...
class POIController:
    ...
    async def upload_person_image(self, person_id: str, organization_id: str, file: UploadFile) -> dict:
        """Upload and process person image with face encoding"""
        try:
            # Validate file type
            if not file.content_type.startswith('image/'):
                raise HTTPException(
